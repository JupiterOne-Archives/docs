...
- The EC2 VPN Gateway to Customer Gateway relationship's outsideIpAddresses
  field has been updated to a concatenated string, delimited by commas.
  Relationship properties cannot be arrays.
- By default, after the API bulk fetches `aws_instance` or `aws_ebs_snapshot`
  Entities, it attempts to get additional detailed information about each Entity
  with another API call per Entity. However, this causes a peformance problem
  when the number of these Entities grows very large. Now, when more than 1000
  `aws_instance` or `aws_ebs_snapshot` Entities are fetched, the integration
  skips the follow-up API calls for additional information. For `aws_instance`
  Entities, this results in the disablement of the 'userData' derived properties
  `secretsDetectedInUserData`, `hasSecrets`, and `secretsDetected`. For
  `aws_ebs_snapshot` Entities, this results in the disablement of the
...
- VPC Peering Relationships occasionally generate "Duplicate Key" errors because
  ...
  sides of the peering. These errors are now caught so that the step doesn't
  fail.
...
- Improved handling of 404 errors for missing group settings.  These errors are caught and handled with 
  a warning message so it doesn't prevent ingestion of existing group settings.
...
- Fixed value of `numericSeverity` by converting the string severity value to a
  number
