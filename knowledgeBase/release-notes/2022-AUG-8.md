...
- Added optional host config field.  The Datadog US host is `datadoghq.us` and the EU host is `datadoghq.eu`
...
Orca Alerts are categorized into the following:
...
- Findings are no longer aggregated by vulnerability ID. This change will allow the finding entities created in J1 to be in parity with the findings in Snyk

The Snyk integration was previously written to aggregate findings based on Vulnerability ID in order to minimize the creation of finding entities that were equivalent. As we have come to see this aggregation cause some issues as additional changes to findings made in Snyk were being lost and the confusion this aggregation caused, we have decided to stop aggregating findings. We will create findings in parity with how they are represented in Snyk which we believe will be easier to understand and query against. This change will result in an increase…

- CWE and CVE entities will be mapped to but we will no longer create these entities from within this integration
...
- Added support for latest version of the Wazuh API and auth method
